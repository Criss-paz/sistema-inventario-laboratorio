...
- [x] Cumplimos los estándares SQL (Sección 6.2) — detalle estándar por estándar en `docs/entrega-2/estandares-cumplimiento.md`, incluida la única excepción declarada (S5, despliegue en internet, planificado para Entrega 4): snake_case, un script por propósito, encabezado con autor/descripción/dependencias en cada archivo, FK con ON DELETE/ON UPDATE documentadas, sin redundancia (normalización 3FN demostrada).
- [x] Cumplimos los estándares de aplicación web aplicables ya en esta entrega (Sección 6.3) — los 6 verificados en `docs/entrega-2/estandares-cumplimiento.md`: separación de capas (config/db/rutas/plantillas), consultas parametrizadas, manejo de errores sin exponer SQL crudo, validación de entrada. (El checklist oficial marca el bloque completo de estándares de app para Entrega 4 — aquí se adelantó lo que ya corresponde a esta entrega.)
